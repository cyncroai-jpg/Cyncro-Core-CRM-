...
function CRMForms({onFlash}:{onFlash:(message:string)=>void}){
  ...
  const generateWithAi=async()=>{
    ...
    setGenerating(true);
    const r=await fetch("/api/crm/forms/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({description:aiPrompt})});
    const d=await r.json() as {fields?:CyncroFormField[];error?:string};
    setGenerating(false);
    if(!r.ok||d.error){onFlash(d.error||"AI generation failed");return}
    setDraft({...draft,fields:d.fields||[]});
    onFlash("AI generated "+(d.fields?.length||0)+" question(s) — review and save");
  };
  ...
  const share=async(row:CyncroFormRow)=>{const url=`${location.origin}/f?form=${row.public_token}`;await navigator.clipboard?.writeText(url);onFlash("Client form link copied")};
...
function PublicBookingExperience() {
  type CustomQuestion = { id: string; label: string; type: string; required: boolean; options?: string[] };
  type EventType = {
    id: string;
    name: string;
    slug: string;
                ...
                <div className="bookingDetails">
                  {customQuestions.map(q => (
                    <div key={q.id} style={{display:"flex",flexDirection:"column",gap:4}}>
                      <label style={{fontSize:11,fontWeight:600,color:"#C8C0C4",letterSpacing:"0.04em"}}>
                        {q.label}{q.required && <span style={{color:"#C1283E",marginLeft:2}}>*</span>}
                      </label>
                      {q.type === "SELECT" && Array.isArray(q.options) ? (
                        <select value={customAnswers[q.id]||""} onChange={e=>setCustomAnswers({...customAnswers,[q.id]:e.target.value})}>
                          <option value="">Select an option</option>
                          {q.options.map(opt=><option key={opt} value={opt}>{opt}</option>)}
                        </select>
                      ) : q.type === "LONG" ? (
                        <textarea value={customAnswers[q.id]||""} onChange={e=>setCustomAnswers({...customAnswers,[q.id]:e.target.value})} rows={3} placeholder={q.label} />
                      ) : q.type === "CHECKBOX" && Array.isArray(q.options) ? (
                        <div style={{display:"flex",flexDirection:"column",gap:6}}>
                          {q.options.map(opt=>(
                            <label key={opt} style={{display:"flex",alignItems:"center",gap:8,fontSize:12,color:"#C8C0C4",fontWeight:400}}>
                              <input type="checkbox" checked={(customAnswers[q.id]||"").includes(opt)}
                                onChange={e=>{const cur=(customAnswers[q.id]||"").split(",").filter(Boolean);const next=e.target.checked?[...cur,opt]:cur.filter(x=>x!==opt);setCustomAnswers({...customAnswers,[q.id]:next.join(",")});}}
                              />
                              {opt}
                            </label>
                          ))}
                        ...
                        <input type={q.type==="EMAIL"?"email":q.type==="NUMBER"?"number":q.type==="DATE"?"date":"text"}
                          value={customAnswers[q.id]||""} onChange={e=>setCustomAnswers({...customAnswers,[q.id]:e.target.value})}
                          placeholder={q.label} />
                      )}
                    </div>
                  ))}
  ...
  const platformComparison = [
    {
      ...
      cyncro: "Running the whole business · sales, scheduling, delivery, money, intelligence",
      pipedrive: "Sales teams tracking deals",
      highlevel: "Agencies reselling marketing tools",
      clickfunnels: "Creators + marketers selling online",
    },
    {
      capability: "Company workspaces (multi-tenant)",
      cyncro: "Native · every company fully isolated, own team + roles",
      pipedrive: "One company per account",
      highlevel: "Agency sub-accounts",
      clickfunnels: "One workspace per account",
    },
    {
    ...
    {
      capability: "Tasks, subtasks + team comments",
      cyncro: "Built in · linked to contacts, deals + accounts",
      pipedrive: "Built in · activities",
      highlevel: "Built in · basic tasks",
      clickfunnels: "Not a core workflow",
    },
    {
      capability: "Booking calendars + links",
      cyncro: "Built in · multi-host, capacity, deposits, waitlists, resources",
      pipedrive: "Built in · basic scheduler",
      highlevel: "Built in · round-robin calendars",
      clickfunnels: "Built in · basic appointments",
    },
    {
      capability: "AI slot ranking (SmartSlot™)",
      cyncro: "Built in · scores every open slot by intent + demand",
      pipedrive: "Not available",
      highlevel: "Not available",
      clickfunnels: "Not available",
    },
    {
      capability: "Rep routing (Outcome Routing™)",
      cyncro: "Built in · round-robin, weighted, performance, availability",
      pipedrive: "Not available",
      highlevel: "Built in · round-robin only",
      clickfunnels: "Not available",
    },
    {
      capability: "Google Calendar sync + ICS feed",
      cyncro: "Built in",
      pipedrive: "Built in",
      highlevel: "Built in",
      clickfunnels: "Via integration",
    },
    {
      capability: "Find new businesses to sell to",
      cyncro: "Built in · Prospecting AI searches any market, scores 0–100",
      pipedrive: "Paid add-on · Prospector contact database",
      highlevel: "Not available",
      clickfunnels: "Not available",
    },
    {
      capability: "Website + contact intelligence on prospects",
      cyncro: "Built in · scans site, finds emails, phones, gaps, ad spend",
      pipedrive: "Not available",
      highlevel: "Not available",
      clickfunnels: "Not available",
    },
    {
      capability: "AI-written call openers + outreach",
      cyncro: "Built in · per-prospect, grounded in real findings",
      pipedrive: "AI email assistant",
      highlevel: "AI content assistant",
      clickfunnels: "AI copy assistant",
    },
    {
      capability: "AI agent workforce",
      cyncro: "12 governed agents · summarize, score, draft, orchestrate",
      pipedrive: "Single AI assistant",
      highlevel: "Conversation + voice AI add-ons",
      clickfunnels: "Not available",
    },
    {
    ...
    {
      capability: "Drag-and-drop forms",
      cyncro: "Built in · e-signature, file upload, CRM write-back",
      pipedrive: "Built in · web forms",
      highlevel: "Built in",
      clickfunnels: "Built in",
    },
    {
      capability: "Drag-and-drop landing pages",
    ...
    },
    {
      ...
      cyncro: "Built in · first/last/linear/time-decay, ad spend, ROI",
      pipedrive: "Not available",
      highlevel: "Basic source reporting",
      clickfunnels: "Basic source reporting",
    },
    {
      capability: "Rep commissions + payout tracking",
      cyncro: "Built in · custom rules, residuals, payout export",
      pipedrive: "Not available",
      highlevel: "Not available",
      clickfunnels: "Affiliate payouts only",
    },
    {
      capability: "Invoices + online payment",
      cyncro: "Built in · Stripe checkout links",
      pipedrive: "Via integration",
      highlevel: "Built in",
      clickfunnels: "Built in · checkout",
    },
    {
      capability: "Contracts + e-signature",
      cyncro: "Built in · multi-signer, versions, audit trail, countersign",
      pipedrive: "Paid add-on · Smart Docs",
      highlevel: "Built in",
      clickfunnels: "Via integration",
    },
    {
      capability: "Sales playbooks + scripts",
      cyncro: "Built in · call/SMS/email, usage + win tracking",
      pipedrive: "Not available",
      highlevel: "Snippets only",
      clickfunnels: "Not available",
    },
    {
      capability: "Social DM + comment automation",
      cyncro: "Built in · Instagram + Facebook keyword flows",
      pipedrive: "Not available",
      highlevel: "Built in",
      clickfunnels: "Not available",
    },
    {
      capability: "Unified team inbox (SMS, email, social)",
      cyncro: "Cyncro Messages · coming soon",
      pipedrive: "Email only",
      highlevel: "Built in",
      clickfunnels: "Email only",
    },
    {
      capability: "Field service dispatch + routing",
      cyncro: "Built in · Cyncro Dispatch",
      pipedrive: "Not available",
      highlevel: "Not available",
      clickfunnels: "Not available",
    },
    {
      capability: "Automotive finance desk",
      cyncro: "Built in · Cyncro Finance",
      pipedrive: "Not available",
      highlevel: "Not available",
      clickfunnels: "Not available",
    },
    {
      capability: "Lending-broker lender network",
      cyncro: "Cyncro Funds · coming soon",
      pipedrive: "Not available",
      highlevel: "Not available",
      clickfunnels: "Not available",
    },
    {
      capability: "Dispute operations + compliance",
      cyncro: "Built in · Cyncro Dispute",
      pipedrive: "Not available",
      highlevel: "Not available",
      clickfunnels: "Not available",
    },
    {
      capability: "API, webhooks + AI booking endpoint",
      cyncro: "Built in · signed webhooks, agent booking API",
      pipedrive: "Built in · API + webhooks",
      highlevel: "Built in · API + webhooks",
      clickfunnels: "Via Zapier",
    },
    {
      capability: "Audit log + role permissions",
      cyncro: "Built in · owner/admin/manager/user/viewer, full audit trail",
      pipedrive: "Built in",
      highlevel: "Built in",
      clickfunnels: "Basic team roles",
    },
  ];
  const cyncroLayers = [
    [
    ...
    <div className="frontExperience">
      ...
      >
        <div className="comparisonIntro">
          ...
          <p>
            Pipedrive is a great deal tracker. ClickFunnels is a great funnel
            builder. HighLevel is a great agency toolkit. Each one covers a
            piece of the job—then hands you off to integrations for the rest.
            Cyncro covers the whole lifecycle in one system: find the
            opportunity, book it, close it, deliver the work, move the money,
            and get smarter with every deal.
          </p>
        </div>
...
type AttributionRow = { id:string; source:string; channel:string; campaign?:string; event_type:string; event_value_cents:number; occurred_at:string; contact_name?:string; landing_page?:string; contact_id?:string|null; visitor_id?:string|null };

/** Real multi-touch attribution: splits each conversion's revenue across the touchpoints in that
 * same visitor/contact's journey leading up to it, weighted by the selected model — not a raw sum. */
function computeAttributedRevenueBySource(touches: AttributionRow[], model: string, lookbackDays: number): Record<string, number> {
  const journeys = new Map<string, AttributionRow[]>();
  for (const t of touches) {
    const key = t.contact_id || t.visitor_id || t.id;
    const list = journeys.get(key) || [];
    list.push(t);
    journeys.set(key, list);
  }
  const bySource: Record<string, number> = {};
  const credit = (source: string, amount: number) => { bySource[source] = (bySource[source] || 0) + amount; };
  for (const journey of journeys.values()) {
    const sorted = [...journey].sort((a, b) => new Date(a.occurred_at).getTime() - new Date(b.occurred_at).getTime());
    const conversions = sorted.filter((t) => Number(t.event_value_cents) > 0);
    for (const conversion of conversions) {
      const conversionTime = new Date(conversion.occurred_at).getTime();
      const windowStart = conversionTime - lookbackDays * 86_400_000;
      const path = sorted.filter((t) => {
        const time = new Date(t.occurred_at).getTime();
        return time <= conversionTime && time >= windowStart;
      });
      const touchpoints = path.length ? path : [conversion];
      const value = Number(conversion.event_value_cents);
      if (model === "FIRST_TOUCH") {
        credit(touchpoints[0].source || "Direct", value);
      } else if (model === "LAST_TOUCH") {
        credit(touchpoints[touchpoints.length - 1].source || "Direct", value);
      } else if (model === "POSITION_BASED" && touchpoints.length > 1) {
        credit(touchpoints[0].source || "Direct", value * 0.4);
        credit(touchpoints[touchpoints.length - 1].source || "Direct", value * 0.4);
        const middle = touchpoints.slice(1, -1);
        if (middle.length) for (const t of middle) credit(t.source || "Direct", (value * 0.2) / middle.length);
        else credit(touchpoints[touchpoints.length - 1].source || "Direct", value * 0.2);
      } else if (model === "TIME_DECAY") {
        const halfLifeMs = 7 * 86_400_000;
        const weights = touchpoints.map((t) => Math.pow(2, -(conversionTime - new Date(t.occurred_at).getTime()) / halfLifeMs));
        const totalWeight = weights.reduce((s, w) => s + w, 0) || 1;
        touchpoints.forEach((t, i) => credit(t.source || "Direct", value * (weights[i] / totalWeight)));
      } else {
        // LINEAR and CUSTOM (no custom weight editor yet, so treated as an even split)
        touchpoints.forEach((t) => credit(t.source || "Direct", value / touchpoints.length));
      }
    }
  }
  return bySource;
...
function CRMAttribution({ onFlash }: { onFlash:(message:string)=>void }) {
  ...
  const attributedRevenue = computeAttributedRevenueBySource(touches, model, lookback);
  const conversionsWithValue = touches.filter((t) => Number(t.event_value_cents) > 0).sort((a, b) => new Date(a.occurred_at).getTime() - new Date(b.occurred_at).getTime());
  const forecast = (() => {
    if (conversionsWithValue.length < 4) return null;
    const midpoint = Math.floor(conversionsWithValue.length / 2);
    const earlier = conversionsWithValue.slice(0, midpoint).reduce((s, t) => s + Number(t.event_value_cents), 0);
    const later = conversionsWithValue.slice(midpoint).reduce((s, t) => s + Number(t.event_value_cents), 0);
    if (earlier <= 0) return null;
    const growthRate = (later - earlier) / earlier;
    return Math.max(0, Math.round(later * (1 + growthRate)));
  })();
  const sources=Object.values(touches.reduce<Record<string,{name:string,revenue:number,touches:number,conversions:number}>>((acc,item)=>{const key=item.source||"Direct";acc[key]??={name:key,revenue:0,touches:0,conversions:0};acc[key].touches++;if(Number(item.event_value_cents)>0)acc[key].conversions++;return acc},{})).map((row)=>({...row,revenue:attributedRevenue[row.name]||0})).sort((a,b)=>b.revenue-a.revenue);
  ...
  return <section className="attributionOS">
    ...
    <div className="attributionControls"><label>Attribution model<select value={model} onChange={e=>void saveModel(e.target.value)}><option value="FIRST_TOUCH">First touch</option><option value="LAST_TOUCH">Last touch</option><option value="LINEAR">Linear</option><option value="TIME_DECAY">Time decay</option><option value="POSITION_BASED">Position based</option><option value="CUSTOM">Custom weighted model</option></select></label><label>Conversion window<select value={lookback} onChange={e=>setLookback(Number(e.target.value))}><option value="30">30 days</option><option value="60">60 days</option><option value="90">90 days</option><option value="180">180 days</option><option value="365">365 days</option></select></label><button onClick={()=>onFlash("Custom weighted models aren't configurable yet — pick a built-in model above")}>Edit custom rules</button><span>● First-party collection active</span></div>
...
function Admin({
  ...
  const loadAuditLog = async (entityId: string) => {
    try {
      const res = await fetch(`/api/calendar/audit-log?entityId=${encodeURIComponent(entityId)}&limit=10`);
      if (res.ok) setAuditLog(((await res.json()) as {log?:typeof auditLog}).log || []);
    } catch { /* non-fatal */ }
  };
  ...
  "agentId": "my-voice-bot"
                ...
                <div key={dayStr} className={`calSlotDayCol ${isToday?"today":""}`}>
                  ...
                  <div className="calSlotDayBody">
                    ...
                    {dayBookings.map(b=>{
                      ...
                      return(<button key={b.id} className="calSlotBooking" onClick={()=>openBookingDetail(b)}
      ...
      {(calendarView as string) === "REVENUE" && (
        <div className="revenueCalendar">
          <div className="revenueCalHead">
            <div>
              <small>REVENUE CALENDAR™</small>
              <b>Appointments with business context — lead score, opportunity, and close potential</b>
            </div>
            <div style={{display:"flex",gap:8}}>
              <span style={{fontSize:11,color:"#7a6e70"}}>{rows.filter(b=>b.status!=="CANCELLED").length} active bookings</span>
            </div>
          </div>
          <div className="revenueCalGrid">
            {(["HIGH","MEDIUM","LOW"] as const).map(tier=>{
              const tierRows=rows.filter(b=>{
                const ls=(b as unknown as Record<string,unknown>).lead_score as number|undefined;
                const score=ls||0;
                if(tier==="HIGH") return score>=70&&b.status!=="CANCELLED";
                if(tier==="MEDIUM") return score>=30&&score<70&&b.status!=="CANCELLED";
                return score<30&&b.status!=="CANCELLED";
              });
              const tierColor=tier==="HIGH"?"#3dcc7a":tier==="MEDIUM"?"#d4c040":"#b8abad";
              const tierLabel=tier==="HIGH"?"High Value (70–100)":tier==="MEDIUM"?"Mid-Market (30–69)":"Nurture (0–29)";
              return(
                <div key={tier} className="revenueTierColumn">
                  <div className="revenueTierHead" style={{borderTopColor:tierColor}}>
                    <span style={{color:tierColor,fontSize:9,fontWeight:700,letterSpacing:"0.1em"}}>{tierLabel}</span>
                    <b style={{color:tierColor}}>{tierRows.length}</b>
                  </div>
                  {tierRows.slice(0,20).map(b=>{
                    const opp=(b as unknown as Record<string,unknown>).opportunity_id as string|null;
                    return(
                      <button key={b.id} className="revenueBookingCard" onClick={()=>openBookingDetail(b)}>
                        <div className="revenueCardHead">
                          <b>{b.customer_name}</b>
                          <span className={`bookingStatusChip status-${b.status.toLowerCase()}`}>{b.status}</span>
                        </div>
                        <small>{b.event_name}</small>
                        <div className="revenueCardMeta">
                          <span>🕐 {new Date(b.starts_at).toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}</span>
                          <span>📍 {b.location_mode.replace("_"," ")}</span>
                          {opp && <span style={{color:"#70a0d0"}}>🔗 Opportunity linked</span>}
                          {b.assigned_to && <span>👤 {b.assigned_to.split("@")[0]}</span>}
                        </div>
                      </button>
                    );
                  })}
                  {!tierRows.length && <div style={{color:"#5a4e51",fontSize:11,padding:"16px 0",textAlign:"center"}}>No bookings in this tier</div>}
                </div>
              );
            })}
          </div>
      ...
      {(calendarView as string) === "TEAM" && (
        <div className="teamCalendar">
          <div className="teamCalHead">
            <small>TEAM CALENDAR — all reps side by side</small>
            <b>{new Date(`${focusDate}T12:00:00`).toLocaleDateString(undefined,{month:"long",day:"numeric",year:"numeric"})}</b>
          </div>
          {(()=>{
            const repSet=new Set(rows.map(b=>b.assigned_to||"Unassigned"));
            const reps=Array.from(repSet).slice(0,6);
            return(
              <div className="teamCalGrid" style={{gridTemplateColumns:`repeat(${Math.max(1,reps.length)},1fr)`}}>
                {reps.map(rep=>{
                  const repBookings=rows.filter(b=>(b.assigned_to||"Unassigned")===rep&&b.status!=="CANCELLED");
                  const today=new Date(`${focusDate}T12:00:00`);
                  const weekStart=new Date(today);weekStart.setDate(today.getDate()-today.getDay());
                  const weekEnd=new Date(weekStart);weekEnd.setDate(weekStart.getDate()+7);
                  const thisWeek=repBookings.filter(b=>new Date(b.starts_at)>=weekStart&&new Date(b.starts_at)<weekEnd);
                  const completed=repBookings.filter(b=>b.status==="COMPLETED").length;
                  const showRate=repBookings.length>0?Math.round(((repBookings.length-repBookings.filter(b=>b.status==="NO_SHOW").length)/repBookings.length)*100):100;
                  return(
                    <div key={rep} className="teamRepColumn">
                      <div className="teamRepHead">
                        <div className="teamRepAvatar">{(rep.split("@")[0]||"?")[0].toUpperCase()}</div>
                        <div>
                          <b>{rep.split("@")[0]}</b>
                          <small>{thisWeek.length} this week · {showRate}% show</small>
                        </div>
                      </div>
                      <div className="teamRepStats">
                        <span><b>{completed}</b><small>done</small></span>
                        <span><b>{repBookings.filter(b=>b.status==="CONFIRMED"||b.status==="RESCHEDULED").length}</b><small>upcoming</small></span>
                        <span><b>{showRate}%</b><small>show rate</small></span>
                      </div>
                      {thisWeek.slice(0,8).map(b=>(
                        <button key={b.id} className="teamRepBooking" onClick={()=>openBookingDetail(b)}>
                          <b>{new Date(b.starts_at).toLocaleString([],{weekday:"short",hour:"numeric",minute:"2-digit"})}</b>
                          <span>{b.customer_name}</span>
                          <small className={`bookingStatusChip status-${b.status.toLowerCase()}`}>{b.status}</small>
                        </button>
                      ))}
                      {!thisWeek.length&&<div style={{color:"#5a4e51",fontSize:11,padding:12,textAlign:"center"}}>No bookings this week</div>}
                    </div>
                  );
                })}
              </div>
      ...
      >
        ...
        {visibleRows.map((booking) => (
          <div className="tr" key={booking.id}>
            ...
            <button onClick={() => openBookingDetail(booking)}>•••</button>
          </div>
        ))}
          ...
          >
            ...
            <div className="bookingfacts">
              <div>
                ...
                <span style={{fontSize:10,color:"#7a6e70"}}>→ {new Date(selectedBooking.ends_at).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})}</span>
              </div>
              <div>
                <small>EVENT</small>
              ...
              <div>
                <small>STATUS</small>
                <b className={`bookingStatusChip status-${selectedBooking.status.toLowerCase()}`}>{selectedBooking.status}</b>
              </div>
              <div>
              ...
              <div>
                <small>CONTACT</small>
                <b>{selectedBooking.customer_email}</b>
                {selectedBooking.customer_phone && <span style={{fontSize:10,color:"#7a6e70",display:"block"}}>{selectedBooking.customer_phone}</span>}
              </div>
              {(selectedBooking as unknown as Record<string,unknown>).lead_score !== undefined && Number((selectedBooking as unknown as Record<string,unknown>).lead_score) > 0 && (
                <div>
                  <small>LEAD SCORE</small>
                  <b style={{color:Number((selectedBooking as unknown as Record<string,unknown>).lead_score)>=70?"#3dcc7a":Number((selectedBooking as unknown as Record<string,unknown>).lead_score)>=40?"#d4c040":"#b8abad"}}>
                    {String((selectedBooking as unknown as Record<string,unknown>).lead_score)}/100
                  </b>
                </div>
              )}
              {Boolean((selectedBooking as unknown as Record<string,unknown>).opportunity_id) && (
                <div>
                  <small>OPPORTUNITY</small>
                  <b style={{color:"#70a0d0"}}>🔗 Linked to CRM</b>
                </div>
              )}
              {selectedBooking.notes && (
                <div style={{gridColumn:"1/-1"}}>
                  <small>NOTES</small>
                  <b style={{fontWeight:400,fontSize:12,color:"#b8abad"}}>{selectedBooking.notes}</b>
                </div>
              )}
            </div>
            ...
            {auditLog.length > 0 && (
              <div className="bookingAuditLog">
                ...
                {auditLog.map(entry=>(
                  <div key={entry.id} className="auditLogEntry">
                    <span className="auditAction">{entry.action}</span>
                    <span className="auditActor">{entry.actor.split("@")[0]}</span>
                    <span className="auditTime">{new Date(entry.created_at).toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {resourcesOpen && (
        <div className="modalback" onClick={() => setResourcesOpen(false)}>
          <div className="bookingmodal pipelineSettings" style={{maxWidth:600,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            <div className="modalhead">
              <div><label>UNIVERSAL RESOURCE GRAPH™</label><h2>Rooms, vehicles &amp; equipment</h2></div>
              <button onClick={() => setResourcesOpen(false)}>×</button>
            </div>
            <p style={{fontSize:12,color:"#b8abad",margin:"0 0 16px"}}>Resources are linked to bookings to prevent double-booking of shared spaces, vehicles, and equipment.</p>
            <div className="crmForm" style={{marginBottom:16}}>
              <label>Name<input value={newResource.name} onChange={e=>setNewResource({...newResource,name:e.target.value})} placeholder="Conference Room A" /></label>
              <label>Type
                <select value={newResource.resourceType} onChange={e=>setNewResource({...newResource,resourceType:e.target.value})}>
                  <option value="ROOM">Room</option><option value="VEHICLE">Vehicle</option><option value="EQUIPMENT">Equipment</option><option value="PERSON">Person</option><option value="VIRTUAL">Virtual</option>
                </select>
              </label>
              <label>Capacity<input type="number" min="1" max="500" value={newResource.capacity} onChange={e=>setNewResource({...newResource,capacity:Number(e.target.value)})} /></label>
              <label>Location<input value={newResource.location} onChange={e=>setNewResource({...newResource,location:e.target.value})} placeholder="Floor 2, Building A" /></label>
              <label>Color<input type="color" value={newResource.color} onChange={e=>setNewResource({...newResource,color:e.target.value})} style={{width:40,height:36,border:"none",background:"none"}} /></label>
            </div>
            <button style={{marginBottom:16,background:"#0a100a",border:"1px solid #253525",color:"#70c080",borderRadius:8,padding:"8px 16px",fontSize:12,fontWeight:600,cursor:"pointer"}} onClick={async()=>{
              if(!newResource.name){setNotice("Resource name is required.");return;}
              const res=await fetch("/api/calendar/resources",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(newResource)});
              if(!res.ok){const d=await res.json() as {error?:string};setNotice(d.error||"Could not create resource.");return;}
              setNewResource({name:"",resourceType:"ROOM",capacity:1,location:"",color:"#C1283E"});
              await loadResources();
              setNotice("Resource added to Universal Resource Graph™");
            }}>+ Add resource</button>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {resources.map(r=>(
                <div key={r.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:"#100e0f",border:"1px solid #2e2527",borderRadius:8}}>
                  <span style={{width:10,height:10,borderRadius:"50%",background:r.color,flexShrink:0}} />
                  <div style={{flex:1}}>
                    <b style={{fontSize:12}}>{r.name}</b>
                    <small style={{fontSize:10,color:"#7a6e70",display:"block"}}>{r.resource_type} · cap {r.capacity}{r.location?` · ${r.location}`:""}</small>
                  </div>
                  <button onClick={async()=>{
                    await fetch("/api/calendar/resources",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:r.id,active:false})});
                    await loadResources();
                  }} style={{color:"#e05060",background:"none",border:"none",cursor:"pointer",fontSize:11}}>Archive</button>
                </div>
              ))}
              {!resources.length && <p style={{color:"#5a4e51",fontSize:12}}>No resources yet. Add rooms, vehicles, or equipment above.</p>}
            </div>
            <div className="modalactions"><button onClick={()=>setResourcesOpen(false)}>Done</button></div>
          </div>
        </div>
      )}
      {manualOpen && (
