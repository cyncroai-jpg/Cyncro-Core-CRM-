...
export async function GET(request: Request) {
  try {
    ...
    const rows = await coreDb().prepare("SELECT f.*, (SELECT COUNT(*) FROM crm_form_submissions s WHERE s.form_id=f.id) submission_count FROM crm_forms f WHERE f.tenant_id=? ORDER BY updated_at DESC").bind(tenant.tenantId).all();
...
export async function PATCH(request: Request) {
  try {
    ...
    if (token && b.action === "SUBMIT") {
      ...
      let contact=await coreDb().prepare("SELECT id FROM crm_contacts WHERE lower(email)=? AND tenant_id=?").bind(email, form.tenant_id).first<{id:string}>();
...
export async function DELETE(request: Request) {
  try { await ensureCoreSchema(); const tenant = await requireTenantAction(request, "delete"); if (tenant instanceof Response) return tenant; const id=cleanText(new URL(request.url).searchParams.get("id"),80); if(!id)return Response.json({error:"Form id required."},{status:400});
    const owned = await coreDb().prepare("SELECT id FROM crm_forms WHERE id=? AND tenant_id=?").bind(id, tenant.tenantId).first();
    if (!owned) return Response.json({error:"Form not found."},{status:404});
