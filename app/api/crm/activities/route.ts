...
export async function GET(request: Request) {
  try {
    ...
    const statement = contactId
      ? db.prepare(`SELECT x.*, c.full_name AS contact_name FROM crm_activities x JOIN crm_contacts c ON c.id=x.contact_id WHERE x.contact_id=? AND c.tenant_id=? ORDER BY x.created_at DESC LIMIT ?`).bind(contactId, tenant.tenantId, limit)
      : db.prepare(`SELECT x.*, c.full_name AS contact_name FROM crm_activities x JOIN crm_contacts c ON c.id=x.contact_id WHERE c.tenant_id=? ORDER BY x.created_at DESC LIMIT ?`).bind(tenant.tenantId, limit);
...
export async function POST(request: Request) {
  try {
    ...
    await coreDb().prepare(`INSERT INTO crm_activities (id,contact_id,activity_type,title,details,due_at,status,created_by,tenant_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(id, contactId, activityType, title, cleanText(body.details, 5000) || null, cleanText(body.dueAt, 60) || null, cleanText(body.status, 30).toUpperCase() || (activityType === "TASK" ? "OPEN" : "COMPLETED"), tenant.email, tenant.tenantId, now, now).run();
...
export async function PATCH(request: Request) {
  try {
    ...
    const owned = await coreDb().prepare("SELECT x.id FROM crm_activities x JOIN crm_contacts c ON c.id=x.contact_id WHERE x.id=? AND c.tenant_id=?").bind(id, tenant.tenantId).first();
    if (!owned) return Response.json({ error: "Activity not found." }, { status: 404 });
