...
export async function GET(request: Request) {
  try {
    ...
    const result = await coreDb().prepare(`SELECT t.*,c.full_name contact_name,a.name account_name,o.name opportunity_name,
      ...
      LEFT JOIN crm_opportunities o ON o.id=t.opportunity_id WHERE t.tenant_id=? ORDER BY CASE t.priority WHEN 'URGENT' THEN 1 WHEN 'HIGH' THEN 2 WHEN 'MEDIUM' THEN 3 ELSE 4 END,t.due_at,t.created_at DESC`).bind(tenant.tenantId).all();
...
export async function POST(request: Request) {
  try {
    ...
    await coreDb().prepare(`INSERT INTO work_tasks (id,title,details,status,priority,assignee,reporter,contact_id,opportunity_id,account_id,due_at,start_at,estimated_minutes,recurrence,dependency_id,tenant_id,created_at,updated_at)
      ...
      cleanText(body.assignee,160)||null,tenant.email,cleanText(body.contactId,80)||null,cleanText(body.opportunityId,80)||null,cleanText(body.accountId,80)||null,
      cleanText(body.dueAt,40)||null,cleanText(body.startAt,40)||null,Math.max(0,Number(body.estimatedMinutes||30)),cleanText(body.recurrence,80)||null,cleanText(body.dependencyId,80)||null,tenant.tenantId,now,now).run();
...
export async function PATCH(request: Request) {
  try {
    ...
    const owned = await db.prepare("SELECT id FROM work_tasks WHERE id=? AND tenant_id=?").bind(id, tenant.tenantId).first();
    ...
    if(action==="COMMENT") { const text=cleanText(body.body,2000); if(!text) return Response.json({error:"Comment required."},{status:400}); await db.prepare("INSERT INTO work_comments (id,task_id,author,body,created_at) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(),id,tenant.email,text,now).run(); return Response.json({saved:true}); }
...
export async function DELETE(request: Request) {
  try {
    ...
    if (tenant instanceof Response) return tenant;
    const id=cleanText(new URL(request.url).searchParams.get("id"),80); if(!id)return Response.json({error:"Task id required."},{status:400});
    const db=coreDb();
    const owned = await db.prepare("SELECT id FROM work_tasks WHERE id=? AND tenant_id=?").bind(id, tenant.tenantId).first();
    if (!owned) return Response.json({error:"Task not found."},{status:404});
    await db.batch([db.prepare("DELETE FROM work_comments WHERE task_id=?").bind(id),db.prepare("DELETE FROM work_subtasks WHERE task_id=?").bind(id),db.prepare("DELETE FROM work_tasks WHERE id=?").bind(id)]); return Response.json({deleted:true});
  }
