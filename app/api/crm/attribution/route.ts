  ...
  try {
    ...
    const [touchpoints, spend, settings, print, reports] = await Promise.all([
      db.prepare(`SELECT t.*,c.full_name contact_name,o.name opportunity_name FROM attribution_touchpoints t
        ...
        WHERE t.tenant_id=? ORDER BY t.occurred_at DESC LIMIT 250`).bind(tenant.tenantId).all(),
      db.prepare("SELECT * FROM attribution_spend WHERE tenant_id=? ORDER BY period_end DESC LIMIT 100").bind(tenant.tenantId).all(),
      db.prepare("SELECT * FROM attribution_settings WHERE workspace_key=?").bind(tenant.tenantId).first(),
      db.prepare("SELECT * FROM attribution_print_campaigns WHERE tenant_id=? ORDER BY created_at DESC").bind(tenant.tenantId).all(),
      db.prepare("SELECT * FROM attribution_reports WHERE tenant_id=? ORDER BY updated_at DESC").bind(tenant.tenantId).all(),
    ]);
...
export async function POST(request: Request) {
  try {
    ...
    if(kind==="REPORT"){
      ...
      await db.prepare("INSERT INTO attribution_reports (id,name,dimensions,metrics,filters,date_range,created_by,tenant_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").bind(crypto.randomUUID(),name,JSON.stringify(Array.isArray(body.dimensions)?body.dimensions:[]),JSON.stringify(Array.isArray(body.metrics)?body.metrics:[]),JSON.stringify(body.filters&&typeof body.filters==="object"?body.filters:{}),cleanText(body.dateRange,20)||"90D","workspace",tenant.tenantId,now,now).run();
    } else if(kind==="PRINT"){
      ...
      await db.prepare("INSERT INTO attribution_print_campaigns (id,name,code,destination_url,distribution_count,tenant_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)").bind(crypto.randomUUID(),name,code,destination,Math.max(0,Number(body.distributionCount||0)),tenant.tenantId,now,now).run();
    } else if (kind === "SPEND") {
      ...
      await db.prepare(`INSERT INTO attribution_spend (id,platform,account_name,campaign,spend_cents,impressions,clicks,period_start,period_end,tenant_id,created_at,updated_at)
        ...
        cleanText(body.periodStart, 30) || now.slice(0,10), cleanText(body.periodEnd, 30) || now.slice(0,10), tenant.tenantId, now, now).run();
    } else {
      await db.prepare(`INSERT INTO attribution_touchpoints (id,visitor_id,contact_id,opportunity_id,channel,source,medium,campaign,content,term,landing_page,referrer,click_id,event_type,event_value_cents,occurred_at,metadata,tenant_id,created_at)
        ...
        JSON.stringify(body.metadata && typeof body.metadata === "object" ? body.metadata : {}), tenant.tenantId, now).run();
    }
...
export async function PATCH(request: Request) {
  try {
    ...
    await coreDb().prepare(`INSERT INTO attribution_settings (workspace_key,model,lookback_days,currency,updated_at) VALUES (?,?,?,?,?)
      ...
      .bind(tenant.tenantId, model, Math.min(365, Math.max(1, Number(body.lookbackDays || 90))), cleanText(body.currency, 8) || "USD", new Date().toISOString()).run();
