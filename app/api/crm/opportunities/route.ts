...
const stages = new Set(["NEW LEAD", "QUALIFIED", "DISCOVERY", "PROPOSAL", "NEGOTIATION", "CLOSED WON", "CLOSED LOST"]);

export async function GET(request: Request) {
  try {
    await ensureCoreSchema();
    const url = new URL(request.url); const rep = cleanText(url.searchParams.get("rep"), 160);
    const db = coreDb();
    const statement = rep
...
export async function POST(request: Request) {
  try {
    ...
    const stage = cleanText(body.stage, 40).toUpperCase() || "NEW LEAD";
    if (!accountId || !name || !stages.has(stage)) return Response.json({ error: "Account, name, and valid stage are required." }, { status: 400 });
    ...
    await coreDb().prepare(`INSERT INTO crm_opportunities
      (id, account_id, primary_contact_id, name, stage, value_cents, probability, assigned_rep, commission_rate_bps,
       commission_status, expected_close_date, source, notes, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING', ?, ?, ?, ?, ?)`)
      .bind(id, accountId, cleanText(body.primaryContactId, 80) || null, name, stage, valueCents, probability,
