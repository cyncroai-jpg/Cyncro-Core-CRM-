  ...
  try {
    ...
    const statement = q
      ? db.prepare(`SELECT a.*, COUNT(DISTINCT c.id) AS contact_count, COUNT(DISTINCT o.id) AS opportunity_count,
          ...
          WHERE a.tenant_id = ? AND (a.name LIKE ? OR a.domain LIKE ? OR a.phone LIKE ?) GROUP BY a.id ORDER BY a.updated_at DESC LIMIT 250`)
          .bind(tenant.tenantId, ...Array(3).fill(`%${q}%`))
      : db.prepare(`SELECT a.*, COUNT(DISTINCT c.id) AS contact_count, COUNT(DISTINCT o.id) AS opportunity_count,
          COALESCE(SUM(o.value_cents),0) AS pipeline_cents, COALESCE(SUM(o.collected_cents),0) AS collected_cents,
          COALESCE(SUM(CAST(o.collected_cents * o.commission_rate_bps AS INTEGER) / 10000),0) AS estimated_payout_cents,
          COALESCE(SUM(CAST(o.collected_cents * o.residual_rate_bps AS INTEGER) / 10000),0) AS monthly_residual_cents FROM crm_accounts a
          LEFT JOIN crm_contacts c ON c.account_id = a.id LEFT JOIN crm_opportunities o ON o.account_id = a.id
          WHERE a.tenant_id = ? GROUP BY a.id ORDER BY a.updated_at DESC LIMIT 250`).bind(tenant.tenantId);
    const results = (await statement.all()).results as Record<string, unknown>[];
    if (tenant.role !== "OWNER") for (const row of results) { delete row.estimated_payout_cents; delete row.monthly_residual_cents; }
...
export async function POST(request: Request) {
  try {
    ...
    await coreDb().prepare(`INSERT INTO crm_accounts
      (id, name, domain, phone, address, category, owner_email, account_manager, sales_director, vp_sales, notes, source, status, tenant_id, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'ACTIVE', ?, ?, ?)`)
      .bind(id, name, cleanText(body.domain, 240) || null, cleanText(body.phone, 40) || null,
        ...
        cleanText(body.ownerEmail, 254) || tenant.email, cleanText(body.accountManager, 160) || null,
        cleanText(body.salesDirector, 160) || null, cleanText(body.vpSales, 160) || null, cleanText(body.notes, 5000) || null, cleanText(body.source, 80) || "MANUAL", tenant.tenantId, now, now).run();
    return Response.json({ account: await coreDb().prepare("SELECT * FROM crm_accounts WHERE id = ? AND tenant_id = ?").bind(id, tenant.tenantId).first() }, { status: 201 });
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    const owned = await coreDb().prepare("SELECT id FROM crm_accounts WHERE id=? AND tenant_id=?").bind(id, tenant.tenantId).first();
    if (!owned) return Response.json({ error: "Account not found." }, { status: 404 });
    ...
    await coreDb().prepare(`UPDATE crm_accounts SET ${fields.join(",")} WHERE id=? AND tenant_id=?`).bind(...values).run();
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return Response.json({ error: "Account id is required." }, { status: 400 });
    const db = coreDb();
    await db.prepare("DELETE FROM crm_accounts WHERE id=? AND tenant_id=?").bind(id, tenant.tenantId).run();
