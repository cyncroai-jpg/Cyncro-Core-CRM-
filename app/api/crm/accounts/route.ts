  ...
  try {
    ...
    const statement = q
      ? db.prepare(`SELECT a.*, COUNT(DISTINCT c.id) AS contact_count, COUNT(DISTINCT o.id) AS opportunity_count,
          COALESCE(SUM(o.value_cents),0) AS pipeline_cents FROM crm_accounts a
...
export async function POST(request: Request) {
  try {
    ...
    await coreDb().prepare(`INSERT INTO crm_accounts
      (id, name, domain, phone, address, category, owner_email, source, status, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'ACTIVE', ?, ?)`)
      .bind(id, name, cleanText(body.domain, 240) || null, cleanText(body.phone, 40) || null,
        ...
        cleanText(body.ownerEmail, 254) || requestUser(request), cleanText(body.source, 80) || "MANUAL", now, now).run();
