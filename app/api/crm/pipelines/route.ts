...
async function ensureDefaultPipeline(tenantId: string) {
  const db = coreDb();
  let pipeline = await db.prepare("SELECT * FROM crm_pipelines WHERE tenant_id = ? AND active = 1 ORDER BY is_default DESC, created_at LIMIT 1").bind(tenantId).first<Record<string, unknown>>();
  if (pipeline) return pipeline;
  const id = crypto.randomUUID(); const now = new Date().toISOString();
  await db.prepare("INSERT INTO crm_pipelines (id, name, description, is_default, active, tenant_id, created_at, updated_at) VALUES (?, 'Sales Pipeline', 'Primary revenue pipeline', 1, 1, ?, ?, ?)")
    .bind(id, tenantId, now, now).run();
  const defaults = [
  ...
  await db.batch(defaults.map((stage, position) => db.prepare(`INSERT INTO crm_pipeline_stages
    ...
    .bind(crypto.randomUUID(), id, stage[0], stage[1], position, stage[2], stage[3], stage[4], tenantId, now, now)));
  await db.prepare("UPDATE crm_opportunities SET pipeline_id = ? WHERE tenant_id = ? AND pipeline_id IS NULL").bind(id, tenantId).run();
...
export async function GET(request: Request) {
  try {
    ...
    await ensureDefaultPipeline(tenant.tenantId);
    const db = coreDb();
    const { results: pipelines } = await db.prepare("SELECT * FROM crm_pipelines WHERE tenant_id = ? AND active = 1 ORDER BY is_default DESC, created_at").bind(tenant.tenantId).all<Record<string, unknown>>();
    const { results: stages } = await db.prepare("SELECT * FROM crm_pipeline_stages WHERE tenant_id = ? ORDER BY pipeline_id, position").bind(tenant.tenantId).all<Record<string, unknown>>();
...
export async function POST(request: Request) {
  try {
    ...
    await db.prepare("INSERT INTO crm_pipelines (id, name, description, is_default, active, tenant_id, created_at, updated_at) VALUES (?, ?, ?, 0, 1, ?, ?, ?)")
      .bind(id, name, cleanText(body.description, 500) || null, tenant.tenantId, now, now).run();
    ...
    await db.batch(stages.map((stage, position) => db.prepare(`INSERT INTO crm_pipeline_stages
      ...
      .bind(crypto.randomUUID(), id, cleanText(stage.name, 80).toUpperCase(), cleanText(stage.color, 20) || "#B51F38", position,
        Math.min(100, Math.max(0, Number(stage.probability || 0))), stage.isWon ? 1 : 0, stage.isLost ? 1 : 0, tenant.tenantId, now, now)));
...
export async function PATCH(request: Request) {
  try {
    ...
    const owned = await db.prepare("SELECT id FROM crm_pipelines WHERE id=? AND tenant_id=?").bind(id, tenant.tenantId).first();
    if (!owned) return Response.json({ error: "Pipeline not found." }, { status: 404 });
    ...
    const statements = [db.prepare("UPDATE crm_pipelines SET name = ?, description = ?, updated_at = ? WHERE id = ? AND tenant_id = ?")
      .bind(name, cleanText(body.description, 500) || null, now, id, tenant.tenantId)];
    for (const oldStage of existingStages) {
      ...
      if (replacement) {
        ...
        if (newName !== oldStage.name) statements.push(db.prepare("UPDATE crm_opportunities SET stage = ? WHERE pipeline_id = ? AND stage = ? AND tenant_id = ?").bind(newName, id, oldStage.name, tenant.tenantId));
      } else if (!incomingIds.has(String(oldStage.id))) {
        statements.push(db.prepare("UPDATE crm_opportunities SET stage = ? WHERE pipeline_id = ? AND stage = ? AND tenant_id = ?").bind(fallbackName, id, oldStage.name, tenant.tenantId));
    ...
    stages.forEach((stage, position) => {
      ...
      if (stage.id && existingStages.some((existing) => existing.id === stage.id)) statements.push(db.prepare(`UPDATE crm_pipeline_stages
        ...
        (id, pipeline_id, name, color, position, probability, is_won, is_lost, tenant_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(crypto.randomUUID(), id, stageName, color, position, probability, stage.isWon || stage.is_won ? 1 : 0, stage.isLost || stage.is_lost ? 1 : 0, tenant.tenantId, now, now));
    });
