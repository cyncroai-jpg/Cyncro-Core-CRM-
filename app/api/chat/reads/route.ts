...
export async function POST(request: Request) {
  try {
    ...
    if (channel.type !== "PUBLIC") {
      const membership = await db.prepare("SELECT 1 FROM team_chat_channel_members WHERE channel_id=? AND member_email=?").bind(channelId, user.email).first();
      if (!membership && user.role !== "OWNER") return Response.json({ error: "Channel access denied." }, { status: 403 });
    }
