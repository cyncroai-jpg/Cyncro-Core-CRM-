...
export async function GET(request: Request) {
  try {
    ...
    // Also return all workspace members for adding to channel
    const { results: allMembers } = await db.prepare("SELECT email, display_name, role FROM workspace_members WHERE active=1 ORDER BY display_name").all<Record<string, unknown>>();
