...
export async function GET(request: Request) {
  try {
    ...
    const feed = token ? await coreDb().prepare("SELECT owner, tenant_id FROM calendar_feeds WHERE token=?").bind(token).first<{owner:string; tenant_id:string}>() : null;
    ...
    const { results } = await coreDb().prepare(`SELECT b.*,e.name AS event_name FROM calendar_bookings b JOIN calendar_event_types e ON e.id=b.event_type_id
      WHERE b.assigned_to=? AND b.tenant_id=? AND b.status IN ('CONFIRMED','RESCHEDULED') ORDER BY b.starts_at`).bind(feed.owner, feed.tenant_id).all<Record<string,unknown>>();
