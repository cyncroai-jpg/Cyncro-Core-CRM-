...
export async function GET(request: Request) {
  try {
    ...
    let query = `SELECT rb.*, r.name AS resource_name, r.resource_type, r.color AS resource_color,
      ...
      WHERE r.tenant_id = ? AND rb.starts_at < ? AND rb.ends_at > ? AND b.status IN ('CONFIRMED','RESCHEDULED')`;
    const params: unknown[] = [tenant.tenantId, to, from];
...
export async function POST(request: Request) {
  try {
    ...
    const db = coreDb();
    const booking = await db.prepare("SELECT starts_at, ends_at FROM calendar_bookings WHERE id = ? AND tenant_id = ?").bind(bookingId, tenant.tenantId).first<{starts_at:string;ends_at:string}>();
    if (!booking) return Response.json({ error: "Booking not found." }, { status: 404 });
    const resource = await db.prepare("SELECT id FROM calendar_resources WHERE id = ? AND tenant_id = ?").bind(resourceId, tenant.tenantId).first();
    ...
    await db.prepare(`INSERT OR IGNORE INTO calendar_resource_bookings (id, booking_id, resource_id, starts_at, ends_at, tenant_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`)
      .bind(id, bookingId, resourceId, booking.starts_at, booking.ends_at, tenant.tenantId, now).run();
...
export async function DELETE(request: Request) {
  try {
    ...
    const resourceId = cleanText(url.searchParams.get("resourceId"), 80);
    if (!bookingId || !resourceId) return Response.json({ error: "bookingId and resourceId are required." }, { status: 400 });
    await coreDb().prepare("DELETE FROM calendar_resource_bookings WHERE booking_id = ? AND resource_id = ? AND tenant_id = ?").bind(bookingId, resourceId, tenant.tenantId).run();
