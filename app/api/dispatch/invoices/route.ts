...
async function jobProfitability(jobId: string) {
  const db = coreDb();
  const job = await db.prepare("SELECT revenue_cents, assigned_tech_id FROM dispatch_jobs WHERE id=?").bind(jobId).first<{ revenue_cents: number; assigned_tech_id: string | null }>();
  if (!job) return null;
...
export async function GET(request: Request) {
  try {
    ...
    if (jobId) {
      const profitability = await jobProfitability(jobId);
...
export async function POST(request: Request) {
  try {
    ...
    const job = await coreDb().prepare("SELECT customer_id, revenue_cents FROM dispatch_jobs WHERE id=?").bind(jobId).first<{ customer_id: string; revenue_cents: number }>();
    if (!job) return Response.json({ error: "Job not found." }, { status: 404 });
    const id = crypto.randomUUID();
    const now = new Date().toISOString();
    await coreDb().batch([
      coreDb().prepare("INSERT INTO dispatch_invoices (id,job_id,customer_id,amount_cents,status,issued_at,created_at,updated_at) VALUES (?,?,?,?,'ISSUED',?,?,?)")
        .bind(id, jobId, job.customer_id, job.revenue_cents, now, now, now),
      coreDb().prepare("UPDATE dispatch_jobs SET status='INVOICED', updated_at=? WHERE id=?").bind(now, jobId),
    ]);
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.markPaid) {
      await coreDb().prepare("UPDATE dispatch_invoices SET status='PAID', paid_at=?, updated_at=? WHERE id=?")
