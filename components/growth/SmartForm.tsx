...
/** The real multi-step smart-form funnel (view/start/step/submit against /api/growth/submit), shared by the standalone /gf page and landing-page FORM sections. */
export default function SmartForm({ token, embedded = false }: { token: string; embedded?: boolean }) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/growth/forms?token=${encodeURIComponent(token)}`).then(async (r) => {
      ...
      setForm(d.form);
      await fetch("/api/growth/submit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, action: "VIEW", ...utm() }) });
    });
  ...
  if (error && !form) return <p className="smartFormError">{error}</p>;
  if (done) return <div className={embedded ? "smartFormEmbedded" : "publicForm giPublic"}><main><i>✓</i><h1>{done.message || "Thanks — we'll be in touch shortly."}</h1></main></div>;
  ...
  return embedded ? <div className="smartFormEmbedded">{body}</div> : <section className="publicForm giPublic"><main>{body}</main></section>;
}
